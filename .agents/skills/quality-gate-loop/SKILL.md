...
metadata:
  ...
  ai4binance.last_reviewed: 2026-09-08
...
On Windows, run the compatibility entry point from the repository root:
...
The compatibility entry point delegates to the canonical
`scripts/quality.ps1 -Profile full` gate. The canonical runner owns test
selection, run-scoped temporary files, cleanup, evidence, and failure reporting.
Exit code zero is the only passing result.
