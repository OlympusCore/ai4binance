---
document_id: AI4B-ARCH-DIAG-406
title: Immutable Shared Snapshot Flow
...
canonical_path: docs/architecture/diagrams/04_features_snapshot/d406_immutable_shared_snapshot_flow.md
...
# Immutable Shared Snapshot Flow
...
| --- | --- |
| Diagram level | `L3` |
| Architecture family | `D400 — FEATURE & SNAPSHOT ARCHITECTURE` |
...
Document the repository-grounded `Immutable Shared Snapshot Flow` boundary and its relationship to the deterministic, fail-closed architecture.
...
sequenceDiagram
 ...
 A->>Q: normalized market data
 Q->>S: one validated snapshot_id
 S-->>C: same immutable snapshot
 C->>E: observations with snapshot_id
 E-->>E: reject mixed lineage
```
