---
document_id: AI4B-ARCH-DIAG-D2601-001
...
canonical_path: docs/architecture/diagrams/26_observability/d2601_observability_architecture.md
...
| --- | --- |
| Diagram level | `L1` |
| Architecture family | `D2600 — OBSERVABILITY` |
...
Document the repository-grounded `Observability Architecture` boundary and its relationship to the deterministic, fail-closed architecture.
...
- `src/ai4binance/ops/decision_telemetry.py`
- `src/ai4binance/agents/telemetry.py`

## 6. Diagram

```mermaid
flowchart LR
 A[Runtime] --> B[Structured Logs]
 A --> C[Metrics]
 A --> D[Audit Events]
 A --> E[Heartbeats]
 B --> F[Local Observability]
 C --> F
 D --> F
 E --> F
 F -->|stale or missing| G[DEGRADED / BLOCKED]
```
