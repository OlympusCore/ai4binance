---
document_id: AI4B-ARCH-DIAG-D1102-001
title: Risk Engine
...
canonical_path: docs/architecture/diagrams/11_risk/d1102_risk_engine.md
...
| --- | --- |
| Diagram level | `L2` |
| Architecture family | `D1100 — RISK ARCHITECTURE` |
...
Document the repository-grounded `Risk Engine` boundary and its relationship to the deterministic, fail-closed architecture.
...
- `src/ai4binance/agents/risk_gate.py`
- `src/ai4binance/governance/risk_assessment.py`
- `schemas/risk/risk_assessment_v2.schema.json`

## 6. Diagram

```mermaid
flowchart LR
 A[DecisionCandidate] --> B[Risk Engine] --> C[RiskAssessment]
 D[Risk Policy] --> B
 E[Market and Liquidity] --> B
 F[Portfolio State] --> B
 C -->|veto| G[NO_TRADE]
 C -->|bounded pass| H[Decision Governance]
 I[LLM] -. cannot override .-> C
```
