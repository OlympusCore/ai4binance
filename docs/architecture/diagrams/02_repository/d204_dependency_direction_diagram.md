---
document_id: AI4B-ARCH-DIAG-204
title: Dependency Direction Diagram
...
canonical_path: docs/architecture/diagrams/02_repository/d204_dependency_direction_diagram.md
...
Document the repository-grounded `Dependency Direction Diagram` boundary and its relationship to the deterministic, fail-closed architecture.
...
```mermaid
flowchart LR
 A[CLI and Interfaces] --> B[Application] --> C[Domain] --> D[Core Contracts]
 E[Infrastructure] --> C
 F[Governance] -. constrains .-> B
 G[Observability] -. observes .-> B
```
...
- [D202 — Modular Monorepo Architecture](d202_modular_monorepo_architecture.md)
