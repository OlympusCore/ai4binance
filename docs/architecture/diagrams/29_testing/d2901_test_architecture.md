---
document_id: AI4B-ARCH-DIAG-D2901-001
...
canonical_path: docs/architecture/diagrams/29_testing/d2901_test_architecture.md
...
| --- | --- |
| Diagram level | `L1` |
| Architecture family | `D2900 — TESTING / QA / QUALITY GATES` |
...
Document the repository-grounded `Test Architecture` boundary and its relationship to the deterministic, fail-closed architecture.
...
- `src/ai4binance/ops/quality_gate`

## 6. Diagram

```mermaid
flowchart LR
 A[Bounded Change] --> B[Targeted Tests] --> C[FAST] --> D[STANDARD] --> E[FULL] --> F[Technical Truth] --> G[Governance Eligibility]
 G --> H{Consequential?}
 H -->|yes| I[Exact-scope Human Approval]
 H -->|no| J[Bounded Baseline]
 I --> J
```
