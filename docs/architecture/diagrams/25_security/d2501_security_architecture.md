---
document_id: AI4B-ARCH-DIAG-D2501-001
...
canonical_path: docs/architecture/diagrams/25_security/d2501_security_architecture.md
...
| --- | --- |
| Diagram level | `L1` |
| Architecture family | `D2500 — SECURITY` |
...
Document the repository-grounded `Security Architecture` boundary and its relationship to the deterministic, fail-closed architecture.
...
- `docs/governance/policy_organization_risk_security_privacy.md`
...
```mermaid
flowchart LR
 A[Trust Boundaries] --> B[Input Validation]
 A --> C[Secret Isolation]
 A --> D[Network Allowlists]
 A --> E[Tool Boundaries]
 B --> F[Security Assurance]
 C --> F
 D --> F
 E --> F
 F -->|finding| G[Quarantine / Deny]
```
...
- `tests/test_private_security.py`
