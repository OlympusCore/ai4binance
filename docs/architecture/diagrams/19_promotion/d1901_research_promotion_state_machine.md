---
document_id: AI4B-ARCH-DIAG-D1901-001
title: Research Promotion State Machine
document_type: REFERENCE
version: 1.0.0
status: ACTIVE
owner: Promotion Governance
...
canonical_path: docs/architecture/diagrams/19_promotion/d1901_research_promotion_state_machine.md
...
# Research Promotion State Machine
...
| --- | --- |
| Diagram level | `L4` |
| Architecture family | `D1900 — PROMOTION LIFECYCLE` |
...
Document the repository-grounded `Research Promotion State Machine` boundary and its relationship to the deterministic, fail-closed architecture.
...
- `src/ai4binance/tuning/promotion.py`
- `src/ai4binance/validation/promotion_evidence.py`
- `src/ai4binance/domain/research/lifecycle.py`
...
stateDiagram-v2
 [*] --> RESEARCH
 RESEARCH --> BACKTESTED
 BACKTESTED --> WF_VALIDATED
 WF_VALIDATED --> OOS_VALIDATED
 OOS_VALIDATED --> ROBUSTNESS_VALIDATED
 ROBUSTNESS_VALIDATED --> PAPER_CANDIDATE
 PAPER_CANDIDATE --> PAPER_APPROVED: human governance
 PAPER_APPROVED --> LIVE_CANDIDATE
 LIVE_CANDIDATE --> LIVE_APPROVED: separate high assurance
 PAPER_APPROVED --> SUSPENDED: drift
```
