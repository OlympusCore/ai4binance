---
document_id: AI4B-ARCH-DIAG-501
title: Canonical Event Fabric
...
canonical_path: docs/architecture/diagrams/05_event_fabric/d501_canonical_event_fabric.md
...
| --- | --- |
| Diagram level | `L1` |
| Architecture family | `D500 — EVENT-DRIVEN ARCHITECTURE` |
...
Document the repository-grounded `Canonical Event Fabric` boundary and its relationship to the deterministic, fail-closed architecture.
...
- `src/ai4binance/events/models.py`
- `src/ai4binance/events/bus.py`
- `src/ai4binance/events/journal.py`
- `schemas/events/event_envelope.schema.json`

## 6. Diagram

```mermaid
flowchart LR
 A[Typed Producer] --> B[EventEnvelope] --> C[Local Event Bus] --> D[Bounded Consumer]
 B --> E[Append-only Journal] --> F[Verified Replay] --> D
 C -->|failure| G[Visible failure boundary]
 F -->|invalid chain| H[REPLAY_BLOCKED]
```
