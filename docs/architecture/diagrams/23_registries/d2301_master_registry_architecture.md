---
document_id: AI4B-ARCH-DIAG-D2301-001
title: Master Registry Architecture
...
canonical_path: docs/architecture/diagrams/23_registries/d2301_master_registry_architecture.md
...
| --- | --- |
| Diagram level | `L1` |
| Architecture family | `D2300 — REGISTRIES` |
...
Document the repository-grounded `Master Registry Architecture` boundary and its relationship to the deterministic, fail-closed architecture.
...
```mermaid
flowchart TB
 A[Registry Governance] --> B[Authority Registry]
 A --> C[Logical Architecture Registry]
 A --> D[Agent Registry]
 A --> E[Strategy Registry]
 A --> F[Model Registry]
 A --> G[Risk Rule Registry]
 A --> H[Evidence Registry]
 A --> I[Workflow Registry]
```
