---
document_id: AI4B-ARCH-DIAG-D2001-001
...
canonical_path: docs/architecture/diagrams/20_continuous_improvement/d2001_continuous_improvement_loop.md
...
| --- | --- |
| Diagram level | `L1` |
| Architecture family | `D2000 — AUTO-CONTROL / AUTO-AUDIT / AUTO-LEARN` |
...
Document the repository-grounded `Continuous Improvement Loop` boundary and its relationship to the deterministic, fail-closed architecture.
...
```mermaid
flowchart LR
 A[Observe] --> B[Auto-Control] --> C[Auto-Audit] --> D[Learning Evidence] --> E[Research Hypothesis] --> F[WF OOS Robustness] --> G[Human Governance]
 G -->|approved| H[Bounded Implementation] --> I[Quality Gate] --> J[Verified Baseline] --> A
 G -->|missing evidence| K[RESEARCH_ONLY]
```
