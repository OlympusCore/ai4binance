---
document_id: AI4B-ARCH-DIAG-104
...
canonical_path: docs/architecture/diagrams/01_governance/d104_governance_control_plane.md
...
Document the repository-grounded `Governance Control Plane` boundary and its relationship to the deterministic, fail-closed architecture.
...
```mermaid
flowchart LR
 A[Governed Object] --> B[Quality Gate] --> C[Governance Gate] --> D{Consequential?}
 D -->|yes| E[Human Governance] --> F{Authorized?}
 D -->|no| G[Evidence-only result]
 F -->|no or unknown| H[RUNNING_WITH_BLOCKERS]
 F -->|exact scope| I[Bounded eligibility]
```
...
- [D101 — Authority Pyramid](d101_authority_pyramid.md)
