---
document_id: AI4B-ARCH-DIAG-D2802-001
...
canonical_path: docs/architecture/diagrams/28_runtime/d2802_process_topology.md
...
| --- | --- |
| Diagram level | `L2` |
| Architecture family | `D2800 — RUNTIME / DEPLOYMENT` |
...
Document the repository-grounded `Process Topology` boundary and its relationship to the deterministic, fail-closed architecture.
...
- `src/ai4binance/skills/continuous_runtime.py`
...
```mermaid
flowchart TB
 A[Scheduled Task or Operator] --> B[Single-instance Supervisor] --> C[Bounded Runtime Cycle]
 C --> D[Market Data]
 C --> E[Virtual Market]
 C --> F[Opportunity Monitor]
 C --> G[Audit and Telemetry]
 D --> H[Repository-owned Runtime]
 E --> H
 F --> H
 G --> H
```
