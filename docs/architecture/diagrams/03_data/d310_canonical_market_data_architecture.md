...
canonical_path: docs/architecture/diagrams/03_data/d310_canonical_market_data_architecture.md
...
| --- | --- |
| Diagram level | `L1` |
| Architecture family | `D300 — DATA PLANE` |
...
Document the repository-grounded `Canonical Market Data Architecture` boundary and its relationship to the deterministic, fail-closed architecture.
...
```mermaid
flowchart LR
 A[Spot] --> D[Bounded Acquisition]
 B[USD-M] --> D
 C[COIN-M] --> D
 D --> E[Normalization] --> F[Quality and Freshness] --> G[MarketSnapshot]
 F -->|invalid or stale| H[DATA_UNAVAILABLE / NO_TRADE]
 G --> I[Checksum Archive]
 G --> J[Bounded Consumers]
```
