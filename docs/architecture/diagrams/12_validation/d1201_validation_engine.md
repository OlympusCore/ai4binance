---
document_id: AI4B-ARCH-DIAG-D1201-001
...
canonical_path: docs/architecture/diagrams/12_validation/d1201_validation_engine.md
...
| --- | --- |
| Diagram level | `L2` |
| Architecture family | `D1200 — VALIDATION ARCHITECTURE` |
...
Document the repository-grounded `Validation Engine` boundary and its relationship to the deterministic, fail-closed architecture.
...
- `src/ai4binance/agents/validation_gate.py`
- `src/ai4binance/validation`
- `schemas/validation/validation_result.schema.json`

## 6. Diagram

```mermaid
flowchart LR
 A[Candidate and Evidence] --> B[Data Validation] --> C[Backtest Integrity] --> D[Walk-forward] --> E[OOS] --> F[Robustness and Regime] --> G[Validation Result]
 G -->|fail or missing| H[NO_TRADE / RESEARCH_ONLY]
 G -->|scope pass| I[Decision Governance]
```
...
- `tests/test_oos_evidence_boundaries.py`
