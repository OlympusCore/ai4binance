---
document_id: AI4B-ARCH-DIAG-D1602-001
...
canonical_path: docs/architecture/diagrams/16_opportunity_observability/d1602_opportunity_lifecycle.md
...
| --- | --- |
| Diagram level | `L4` |
| Architecture family | `D1600 — OPPORTUNITY OBSERVABILITY` |
...
Document the repository-grounded `Opportunity Lifecycle` boundary and its relationship to the deterministic, fail-closed architecture.
...
stateDiagram-v2
 [*] --> OBSERVED
 OBSERVED --> CANDIDATE: eligible evidence
 OBSERVED --> REJECTED: blocker
 CANDIDATE --> WATCH_ONLY: confirmation pending
 CANDIDATE --> REJECTED: invalidated
 WATCH_ONLY --> CLOSED: expiry or review
 REJECTED --> REVIEWED
 CLOSED --> REVIEWED
 REVIEWED --> [*]
```
