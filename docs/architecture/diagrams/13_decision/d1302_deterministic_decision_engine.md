---
document_id: AI4B-ARCH-DIAG-D1302-001
...
canonical_path: docs/architecture/diagrams/13_decision/d1302_deterministic_decision_engine.md
...
Document the repository-grounded `Deterministic Decision Engine` boundary and its relationship to the deterministic, fail-closed architecture.
...
```mermaid
flowchart LR
 A[Setup] --> B[Deterministic Core] --> C[DecisionCandidate]
 D[Snapshot] --> B
 E[Evidence] --> B
 C --> F[RiskAssessment]
 C --> G[Validation]
 B -. does not produce .-> H[TradeDecision]
```
...
- [D1303 — Decision Governance Engine](d1303_decision_governance_engine.md)
