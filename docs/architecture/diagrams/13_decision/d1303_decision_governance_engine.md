---
document_id: AI4B-ARCH-DIAG-D1303-001
...
canonical_path: docs/architecture/diagrams/13_decision/d1303_decision_governance_engine.md
...
| --- | --- |
| Diagram level | `L2` |
| Architecture family | `D1300 — DECISION ARCHITECTURE` |
...
Document the repository-grounded `Decision Governance Engine` boundary and its relationship to the deterministic, fail-closed architecture.
...
```mermaid
flowchart LR
 A[DecisionCandidate] --> D[Decision Governance]
 B[RiskAssessment] --> D
 C[Validation] --> D
 P[Policy and Blockers] --> D
 D -->|all gates| E[TradeDecision]
 D -->|veto or unknown| F[NO_TRADE]
 E --> G[Separate Execution Gate]
```
...
- [D1302 — Deterministic Decision Engine](d1302_deterministic_decision_engine.md)
