---
document_id: AI4B-ARCH-DIAG-901
...
canonical_path: docs/architecture/diagrams/09_evidence/d901_evidence_architecture.md
...
| --- | --- |
| Diagram level | `L1` |
| Architecture family | `D900 — EVIDENCE FABRIC` |
...
Document the repository-grounded `Evidence Architecture` boundary and its relationship to the deterministic, fail-closed architecture.
...
- `src/ai4binance/governance/evidence_contracts.py`
- `src/ai4binance/external_intel/evidence/evidence_graph.py`
- `schemas/evidence/evidence_bundle.schema.json`

## 6. Diagram

```mermaid
flowchart LR
 A[Source] --> B[Claim] --> C[Evidence] --> D[Provenance]
 C --> E[Supporting]
 C --> F[Counter Evidence]
 E --> G[EvidencePack]
 F --> G
 G --> H[Completeness Gate]
 H -->|missing| I[UNKNOWN / RESEARCH_ONLY]
```
...
- `tests/test_enterprise_evidence_boundaries.py`
