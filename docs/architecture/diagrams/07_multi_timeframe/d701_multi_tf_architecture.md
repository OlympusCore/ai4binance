---
document_id: AI4B-ARCH-DIAG-701
title: Multi-TF Architecture
...
canonical_path: docs/architecture/diagrams/07_multi_timeframe/d701_multi_tf_architecture.md
...
| --- | --- |
| Diagram level | `L1` |
| Architecture family | `D700 — MULTI-TIMEFRAME TRADING INTELLIGENCE` |
...
Document the repository-grounded `Multi-TF Architecture` boundary and its relationship to the deterministic, fail-closed architecture.
...
- `src/ai4binance/research/futures_multitf.py`
- `src/ai4binance/data/market_history_continuous.py`
- `src/ai4binance/application/opportunity_monitor.py`

## 6. Diagram

```mermaid
flowchart LR
 U[Eligible Universe] --> A[15m direct] --> S[All-universe screening]
 U --> B[1h direct] --> S
 S --> C[Candidates]
 C --> D[5m enrichment]
 C --> E[4h enrichment]
 C --> F[1d enrichment]
 D --> G[Multi-timeframe evidence]
 E --> G
 F --> G
 G -->|gap or conflict| H[WAIT / NO_TRADE]
```
