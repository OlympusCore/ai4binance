---
document_id: AI4B-ARCH-DIAG-D1403-001
...
canonical_path: docs/architecture/diagrams/14_execution/d1403_paper_execution_architecture.md
...
| --- | --- |
| Diagram level | `L1` |
| Architecture family | `D1400 — EXECUTION ARCHITECTURE` |
...
Document the repository-grounded `Paper Execution Architecture` boundary and its relationship to the deterministic, fail-closed architecture.
...
- `src/ai4binance/execution/ledger.py`

## 6. Diagram

```mermaid
flowchart LR
 A[TradeDecision] --> B[Paper Eligibility] --> C[Paper Intent] --> D[Fill and Cost Simulation] --> E[Ledger] --> F[Virtual Position] --> G[Closure Review]
 B -->|blocked| H[NO_TRADE]
 I[Live Path] -. disabled .-> J[LIVE_ORDER_BLOCKED]
```
