...
class SetupScanDisposition(StrEnum):
    """Default scanner visibility for setup quality tiers."""
...
ZERO = Decimal("0")
FIRST_SETUP_QUALITY_QUESTION = "WHY_NOT_TO_TRADE"


def setup_scan_disposition(tier: SetupTier) -> SetupScanDisposition:
    """Return the canonical scan disposition for a setup quality tier."""
    if tier in {SetupTier.A_STAR, SetupTier.A, SetupTier.B_PLUS}:
        return SetupScanDisposition.DISPLAY
    return SetupScanDisposition.WATCH_ONLY
...
def _normalize_market_type(market_type: str) -> str:
    """Return the canonical market type contract used by deterministic decisions."""
    normalized_market_type = market_type.strip().upper()
    if not normalized_market_type:
        raise ValueError("market_type cannot be empty")
    if normalized_market_type not in {"SPOT", "USD_M_FUTURES"}:
...
class TradeCandidate:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            and self.action is Action.SELL
            and self.inventory_action == "NONE"
        ):
