...
const marketSelections = {SPOT:'',USD_M_FUTURES:''};
  ...
  function marketMonitor(content,virtual=false){
    ...
    for(const tf of visible){
      ...
      if(coverage.status)p.appendChild(el('div','aw-status',t('Canonical opportunity state: ','Kanonik fırsat durumu: ')+monitorLabel(coverage.status)));
      if(coverage.data_blocked_symbol_count)p.appendChild(el('div','aw-status',t('Data-blocked symbols: ','Veri nedeniyle engelli semboller: ')+String(coverage.data_blocked_symbol_count)+t('. Published research opportunities remain visible.','; yayınlanmış araştırma fırsatları görünür kalır.')));
  ...
  function findingCopy(item){
    ...
    const copies={
      ...
      AUTO_AUDIT_OBSERVER_UNAVAILABLE:{title:['Auto-audit observer evidence is unavailable','Otomatik denetim gözlemci kanıtı kullanılamıyor'],explanation:['The dashboard cannot infer system movement without the canonical auto-audit artifact.','Dashboard, kanonik otomatik denetim çıktısı olmadan sistem hareketi çıkarımı yapamaz.'],action:['Restore the canonical auto-audit artifact; do not replace it with dashboard-only logging.','Kanonik otomatik denetim çıktısını geri getirin; dashboard-özel günlükleme ile değiştirmeyin.']}
    };
  ...
  function auditObserverPanel(){
    const observer=localData?.auto_audit||{},movements=observer.movements||[],recommendations=observer.recommendations||[];
    const p=panel(['Observed movements and recommendations','Gözlenen hareketler ve öneriler'],badge(statusLabel(observer.status),observer.status==='READY'?'':'warn'));
    append(p,row(['Observer method','Gözlemci yöntemi'],['Canonical Auto-Audit Loop + Continuous Assurance','Kanonik Otomatik Denetim Döngüsü + Sürekli Güvence']),row(['Audit loop','Denetim döngüsü'],observer.loop_id||'—'),row(['Workflow result','İş akışı sonucu'],observer.workflow_pattern||'—'),row(['Observed movements','Gözlenen hareket'],String(movements.length)));
    if(recommendations.length)p.appendChild(table([['Recommendation','Öneri'],['Method','Yöntem'],['Method result','Yöntem sonucu'],['Observed','Gözlem zamanı'],['Evidence','Kanıt']],recommendations.slice(0,40).map(item=>[item.recommendation||'—',item.method||'—',item.method_result||'—',observedTime(item.observed_at),item.evidence_ref||'—'])));
    else p.appendChild(el('div','aw-empty',['No recommendation is inferred without a canonical auto-audit action reference.','Kanonik otomatik denetim eylem referansı olmadan öneri çıkarılmaz.']));
    if(movements.length){const latest=movements[0];p.appendChild(table([['Cycle','Döngü'],['Result','Sonuç'],['New blockers','Yeni engeller'],['Persistent blockers','Kalıcı engeller'],['Resolved blockers','Çözülen engeller']],movements.slice(0,20).map(item=>[String(item.cycle_index??'—'),item.system_status||'—',(item.new_blockers||[]).join(' · ')||'—',(item.persistent_blockers||[]).join(' · ')||'—',(item.resolved_blockers||[]).join(' · ')||'—'])));}
    return p;
  }
  ...
  function renderLocal(content){
    ...
    if(state.page==='recommendations'){
      content.appendChild(learningToolbar());
      const learning=localData.learning||{},readiness=localData.operational_readiness||{},observer=localData.auto_audit||{};append(content,kpis([[['System health','Sistem sağlığı'],statusLabel(readiness.status),String(readiness.finding_count??'—')+' '+t('findings','bulgu')],[['Observed movements','Gözlenen hareketler'],String((observer.movements||[]).length),statusLabel(observer.status)],[['Recorded lessons','Kaydedilen öğrenimler'],String(learning.lesson_count??'—'),['Canonical learning summary','Kanonik öğrenim özeti']]]));
      content.appendChild(auditObserverPanel());
