...
from ai4binance.content.localization import turkish_market_outlook_content
...
class ContentDraftEngine:
    ...
    def create_market_outlook_draft(self, evidence: EvidenceEnvelope) -> ContentDraft:
        ...
        content = turkish_market_outlook_content(
            symbol=symbol,
            one_day_bias=biases["1d"],
            four_hour_bias=biases["4h"],
            one_hour_bias=biases["1h"],
            regime=regime,
            volatility=volatility,
            radar=radar,
            rationale=rationale,
            disclaimer=self.policy.required_disclaimer,
        )
