...
class AdvancedTechnicalAgent(BaseAgent):
    ...
    def _trend_channel(
        ...
        return self._feature(vote, "LINEAR_CHANNEL_SLOPE", slope=str(slope))
    ...
    def _candlestick(
        ...
        return self._feature(vote, "CONTEXTUAL_ENGULFING", setups=(setup,))

    def _chart_pattern(
        self, candles: tuple[OHLCVCandle, ...], _snapshot: MarketSnapshot
    ) -> _Feature | None:
        values = closes(candles[-50:])
        first = values[:25]
        second = values[25:]
        tolerance = atr(candles[-50:], 14)
        high_gap = abs(max(first) - max(second))
        low_gap = abs(min(first) - min(second))
        if high_gap <= tolerance * Decimal("0.25"):
            return self._feature(-0.5, "RULE_BASED_DOUBLE_TOP", setups=("DOUBLE_TOP",))
        if low_gap <= tolerance * Decimal("0.25"):
            return self._feature(
                0.5, "RULE_BASED_DOUBLE_BOTTOM", setups=("DOUBLE_BOTTOM",)
            )
        return None
    ...
    def _elliott(
        ...
        if not bullish and not bearish:
            return None
        return self._feature(
            0.35 if bullish else -0.35, "RULE_BASED_ALTERNATING_SWINGS"
        )
