...
class StrategyEngine:
    """Generate research-only candidates; never grant execution permission."""
    ...
    def generate(
        ...
        """Generate only the implemented trend-continuation research candidate."""
        ...
        compression_candidates = CompressionBreakoutPlaybookEngine().generate(
            snapshot, agent_results
        )
        regime_candidates = RegimePlaybookEngine().generate(snapshot, agent_results)
        ...
        if not playbook.implemented or snapshot.latest_price is None:
            return self._approved(supplemental_candidates, agent_results)
        ...
        if current_atr <= Decimal("0") or entry <= current_atr * self.atr_multiplier:
            return self._approved(price_action_candidates, agent_results)
        ...
        return self._approved(candidates, agent_results)
    ...
    def _approved(
        ...
        return tuple(self._apply_approval(candidate) for candidate in whale_adjusted)
