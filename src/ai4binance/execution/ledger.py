...
class PaperLedger:
    ...
    def latest_positions(self) -> tuple[LifecyclePosition, ...]:
        if not self.path.exists():
            return ()
        ...
        return tuple(
            self._position_from_payload(latest[position_id])
            for position_id in sorted(latest)
        )

    def closed_positions(self) -> tuple[LifecyclePosition, ...]:
        return tuple(
            position
            for position in self.latest_positions()
            if position.status is PositionStatus.CLOSED
            and position.closure_review is not None
        )

    def _position_payloads(self) -> tuple[dict[str, object], ...]:
        ...
        for line in self.path.read_text(encoding="utf-8").splitlines():
            ...
            if isinstance(position, dict):
                rows.append(cast(dict[str, object], position))
        return tuple(rows)

    def _position_from_payload(self, payload: dict[str, object]) -> LifecyclePosition:
