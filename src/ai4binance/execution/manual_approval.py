...
class ApprovalQueueRecord:
    ...
    created_at: datetime
    execution_authorization: ExecutionAuthorizationEnvelope | None = None
    ...
    def __post_init__(self) -> None:
        ...
        if authorization is not None and (
            self.action.action_type is not ManualActionType.PLACE_SPOT_ORDER
            or authorization.approval_id != self.approval.approval_id
            or authorization.symbol != self.action.asset_or_symbol.strip().upper()
        ):
            raise ValueError("execution authorization does not match approval record")
...
class LocalApprovalQueue:
    ...
    def resolve_execution_authorization(
        ...
        """Resolve the latest unique approved exact-order authorization."""
        ...
        bound = tuple(
            ...
            if record.execution_authorization is not None
            and record.execution_authorization.authorization_id == normalized
        )
        if not bound:
            return None
        approval_ids = {
            record.execution_authorization.approval_id
            for record in bound
            if record.execution_authorization is not None
        }
        ...
        matching = tuple(
            record for record in records if record.approval.approval_id == approval_id
        )
        latest_at = max(record.created_at for record in matching)
        latest = tuple(record for record in matching if record.created_at == latest_at)
        if len(latest) != 1:
            return None
        record = latest[0]
        ...
        if authorization is None:
            return None
        if (
            record.action.approval_status is not ApprovalStatus.APPROVED
            or record.approval.approval_status is not ApprovalStatus.APPROVED
            or authorization.authorization_id != normalized
            or record.action.action_type is not ManualActionType.PLACE_SPOT_ORDER
            or record.approval.action_type is not ManualActionType.PLACE_SPOT_ORDER
            or authorization.approval_id != record.approval.approval_id
            or authorization.symbol != record.action.asset_or_symbol.strip().upper()
        ):
            return None
    ...
    def _record(payload: object) -> ApprovalQueueRecord:
        ...
        authorization = (
            ExecutionAuthorizationEnvelope.from_payload(authorization_payload)
