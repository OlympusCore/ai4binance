...
_FILE_LABELS = {
    "spot/orders.jsonl": ("Spot REST order history", "REST full snapshot"),
    "spot/order_events.jsonl": ("Spot User Data Stream order events", "WebSocket"),
    "spot/trades.jsonl": ("Spot REST trade/fill history", "REST full snapshot"),
    "spot/capital_flows.jsonl": ("Spot capital flows", "REST/WebSocket"),
    "spot/account_snapshots.jsonl": ("Spot account snapshot", "Runtime REST"),
    "spot/balance_snapshots.jsonl": ("Spot balance snapshot", "Runtime REST"),
    "futures_usdm/orders.jsonl": (
        "USD-M Futures REST order history",
        "REST full snapshot",
    ),
    "futures_usdm/order_events.jsonl": ("USD-M Futures order events", "WebSocket"),
    "futures_usdm/trades.jsonl": (
        "USD-M Futures trade/fill history",
        "REST full snapshot",
    ),
    "futures_usdm/positions_current.jsonl": (
        "USD-M Futures open position snapshot",
        "REST full snapshot",
    ),
    "futures_usdm/position_events.jsonl": (
        "USD-M Futures position events",
        "WebSocket",
    ),
    "futures_usdm/income_ledger.jsonl": (
        "USD-M Futures income/funding records",
        "REST full snapshot",
    ),
    "futures_usdm/configuration_snapshots.jsonl": (
        "USD-M Futures configuration",
        "REST full snapshot",
    ),
    "shared/raw_api_events.jsonl": ("Raw API event identifiers", "REST/WebSocket"),
    "shared/reconciliation_results.jsonl": (
        "REST/WebSocket reconciliation results",
        "Reconciliation",
    ),
    "shared/api_sync_runs.jsonl": ("Runtime sync runs", "Runtime REST"),
}
...
def _channel_title(product: str, source: str) -> str:
    ...
    source_label = "REST full snapshot" if source == "REST" else "User Data Stream"
...
<body>
  <header>
    <h1>AI4BINANCE Accounting Report</h1>
    <div class="muted">Production order authority is disabled: LIVE_ORDER_BLOCKED. Source JSON: {html.escape(str(json_path))}</div>
  </header>
  <main>
    <section class="grid">
      <div class="panel"><div class="muted">Status</div><div><span class="badge {status_class}">{html.escape(status)}</span></div></div>
      <div class="panel"><div class="muted">REST records</div><div class="metric">{source_counts.get("REST", 0)}</div></div>
      <div class="panel"><div class="muted">WebSocket records</div><div class="metric">{source_counts.get("WEBSOCKET", 0)}</div></div>
      <div class="panel"><div class="muted">Reconciliation</div><div class="metric">{html.escape(str(reconciliation["status"]))}</div></div>
    </section>
    <section class="panel">
      <h2>REST Snapshot and User Data Stream</h2>
...
def _render_blockers(blockers: tuple[str, ...]) -> str:
    if not blockers:
        return '<p class="muted">No active blockers.</p>'
    return (
        '<ul class="blockers">'
...
def _render_activity(items: tuple[dict[str, object], ...]) -> str:
    ...
    return f"<table><thead><tr><th>Time</th><th>Product</th><th>Source</th><th>Event</th><th>Severity</th></tr></thead><tbody>{rows}</tbody></table>"
...
def _render_channels(channels: tuple[dict[str, object], ...]) -> str:
    ...
    return f"<table><thead><tr><th>Channel</th><th>Status</th><th>Events</th><th>Latest Record</th><th>Age</th><th>Endpoints</th></tr></thead><tbody>{rows}</tbody></table>"
...
def _render_files(files: tuple[dict[str, object], ...]) -> str:
    rows = "".join(
        ...
        f"<td>{'present' if item['exists'] else 'missing'}</td>"
        f"<td>{html.escape(str(item['line_count']))}</td>"
        f"<td>{html.escape(_age(item['age_seconds']))}</td>"
        f"<td>{'yes' if item['stale'] else 'no'}</td>"
        "</tr>"
        for item in files
    )
    return f"<table><thead><tr><th>Record</th><th>Type</th><th>File</th><th>Status</th><th>Lines</th><th>Age</th><th>Stale</th></tr></thead><tbody>{rows}</tbody></table>"
