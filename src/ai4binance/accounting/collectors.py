...
def _futures_algo_order(payload: Mapping[str, object]) -> FuturesAlgoOrderRecord:
    created = _ms_time(
        payload.get("createTime", payload.get("time", payload.get("createdTime"))),
        "createTime",
    )
    return FuturesAlgoOrderRecord(
        ...
        position_side=_position_side(payload.get("positionSide", "BOTH")),
        algo_type=_text(
            payload.get("orderType", payload.get("type", payload.get("algoType"))),
            "orderType",
        ).upper(),
        ...
        activation_price=_optional_decimal(
            payload.get("activatePrice", payload.get("activationPrice")),
            "activatePrice",
        ),
        callback_rate=_optional_decimal(payload.get("callbackRate"), "callbackRate"),
        status=_text(
            payload.get("algoStatus", payload.get("status")), "algoStatus"
        ).upper(),
        created_at=created,
    )
