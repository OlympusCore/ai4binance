...
def _normalize_market_type(market_type: str) -> str:
    """Return the canonical market type used by shared snapshot contracts."""
...
def is_futures_market_type(market_type: str) -> bool:
    """Return whether a market type represents USD-M futures."""
    return _normalize_market_type(market_type) == "USD_M_FUTURES"
...
class AnalysisState:
    ...
    def __post_init__(self) -> None:
        ...
        if self.compiled_cycle_context is not None:
            if self.compiled_cycle_context.market_snapshot_id != self.snapshot_id:
                raise ValueError(
                    "compiled cycle context must reference the shared snapshot"
                )
