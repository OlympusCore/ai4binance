...
@dataclass(frozen=True, slots=True)
class DynamicTrendChannelPoint:
    """One closed-candle dynamic trend channel observation."""
...
def dynamic_trend_channel(
    candles: Sequence[OHLCVCandle],
    lookback: int = 20,
    atr_period: int = 14,
    width_multiplier: Decimal = Decimal("2"),
    slope_threshold: Decimal = ZERO,
) -> tuple[DynamicTrendChannelPoint, ...]:
    """Return an ATR-width linear-regression trend channel from closed candles.

    The latest candle in each window is the observation point. No provisional
    right-edge or future candle is used, so the primitive remains suitable for
    validation-first research and report-only opportunity review.
    """
    if lookback < 2:
        raise ValueError("dynamic trend channel lookback must be at least 2")
    if atr_period < 1:
        raise ValueError("dynamic trend channel atr_period must be positive")
    if width_multiplier <= ZERO:
        raise ValueError("dynamic trend channel width_multiplier must be positive")
    if slope_threshold < ZERO:
        raise ValueError("dynamic trend channel slope_threshold must be non-negative")
    first_index = max(lookback - 1, atr_period)
    if len(candles) <= first_index:
        raise ValueError(
            "dynamic trend channel requires enough closed candles for lookback and ATR"
        )

    output: list[DynamicTrendChannelPoint] = []
    for index in range(first_index, len(candles)):
        candle = candles[index]
        regression_window = candles[index - lookback + 1 : index + 1]
        slope, midline = _linear_regression_latest(
            tuple(item.close for item in regression_window)
        )
        channel_width = (
            atr(candles[index - atr_period : index + 1], atr_period) * width_multiplier
        )
        upper = midline + channel_width
        lower = midline - channel_width
        output.append(
            DynamicTrendChannelPoint(
                ...
                slope=slope,
                width=channel_width,
                trend_direction=_trend_direction(slope, slope_threshold),
                breakout=_channel_breakout(candle.close, upper, lower),
            )
        )
    return tuple(output)
...
def _linear_regression_latest(values: Sequence[Decimal]) -> tuple[Decimal, Decimal]:
    _require_period(values, 2)
    length = Decimal(len(values))
    x_mean = Decimal(len(values) - 1) / Decimal("2")
    y_mean = sum(values, ZERO) / length
    numerator = ZERO
    denominator = ZERO
    for index, value in enumerate(values):
        x_distance = Decimal(index) - x_mean
        numerator += x_distance * (value - y_mean)
        denominator += x_distance * x_distance
    if denominator == ZERO:
        raise ValueError("linear regression requires at least two distinct points")
    slope = numerator / denominator
    intercept = y_mean - (slope * x_mean)
    latest_midline = intercept + (slope * Decimal(len(values) - 1))
    return slope, latest_midline


def _trend_direction(slope: Decimal, threshold: Decimal) -> str:
    if slope > threshold:
        return "UP"
    if slope < -threshold:
        return "DOWN"
...
def _channel_breakout(close: Decimal, upper: Decimal, lower: Decimal) -> str:
    if close > upper:
        return "ABOVE"
    if close < lower:
        return "BELOW"
