...
def _audit_computer_md_privacy_boundary(
    ...
    passed = (
        ...
        and report.computer_profile_ref
    ...
    return _check(
        ...
        "privacy:docs/archive/reference_local_computer_profile.md",
        passed=passed,
        evidence_refs=(
        ...
        corrective_actions=(
            (
                ...
                "docs/archive/reference_local_computer_profile.md boundary.",
            )
            if blockers
            else ()
        ),
...
def _audit_quality_gate_green(
    ...
    quality_gate_marker = "runtime\\artifacts\\quality\\gate\\latest.json"
    governance_gate_marker = (
        "runtime\\artifacts\\quality\\gate\\governance_gate_latest.json"
    )
    if (
        TECHNICAL_QUALITY_PRIMARY_STATUS not in script_text
        or quality_gate_marker not in script_text
    ):
        missing.append("TECHNICAL_QUALITY_PASS_EVIDENCE_WRITER_MISSING")
    if (
        "Invoke-DeterministicGovernanceGate" not in script_text
        or governance_gate_marker not in script_text
    ):
        missing.append("QUALITY_GATE_GOVERNANCE_GATE_MISSING")
    ...
    return _check(
        ...
        evidence_refs=(
            ...
            "artifact:runtime/artifacts/quality/gate/latest.json",
            "artifact:runtime/artifacts/quality/gate/governance_gate_latest.json",
        ),
        ...
        corrective_actions=tuple(
            {
                ...
                "TECHNICAL_QUALITY_PASS_EVIDENCE_WRITER_MISSING": (
                    f"Write {TECHNICAL_QUALITY_PRIMARY_STATUS} evidence only "
                    "after the full technical gate passes."
                ),
                "QUALITY_GATE_GOVERNANCE_GATE_MISSING": (
                    "Run the deterministic governance gate and persist its artifact."
                ),
...
def _audit_holding_governance_doc(workspace_root: Path) -> QualitySystemAuditCheck:
    doc_path = workspace_root / "docs" / "governance/policy_holding_governance.md"
    required_terms = (
    ...
    return _check(
        ...
        evidence_refs=("doc:docs/governance/policy_holding_governance.md",),
        blockers=(
        ...
        corrective_actions=(
            ("Create docs/governance/policy_holding_governance.md.",)
            if not doc_path.is_file()
