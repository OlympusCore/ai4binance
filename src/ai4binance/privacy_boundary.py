"""Fail-closed local privacy boundary for the archived local profile."""
...
class PrivacyBoundaryFinding:
    """A redacted location where local profile information was found."""

    file_path: str
    line_number: int
    token_sha256: str
    evidence_ref: str = "docs/archive/reference_local_computer_profile.md"
...
DEFAULT_EXCLUDED_FILES: frozenset[str] = frozenset(
    {"docs/archive/reference_local_computer_profile.md", "computer_local.md"}
)

_SHA256_HEX_RE = re.compile(r"[0-9a-f]{64}")
_TOKEN_RE = re.compile(r"[A-Za-z0-9][A-Za-z0-9_.:@/\\-]{3,}")
_WINDOWS_PATH_RE = re.compile(r"[A-Za-z]:\\[^\s`\"'<>|]+")
_VERSION_TOKEN_RE = re.compile(r"\d+(?:\.\d+){1,3}(?:[-_][A-Za-z0-9]+)?")
...
def scan_privacy_boundary(
    ...
    *,
    ...
    """Scan repo text files for sensitive local profile tokens.

    The report intentionally contains only token hashes and repository-relative
    file paths. Raw local profile content never leaves the scanner.
    """

    root = repo_root.resolve()
    profile_path = (
        computer_md or root / "docs/archive/reference_local_computer_profile.md"
    ).resolve()
...
def _extract_candidate_tokens(text: str) -> frozenset[str]:
    ...
    for line in text.splitlines():
        ...
        if not any(key in line_lower for key in _SENSITIVE_FIELD_KEYS):
            continue
        for raw in _TOKEN_RE.findall(_candidate_value_segment(line)):
            token = raw.strip("`'\"()[]{}.,;:")
...
def _candidate_value_segment(line: str) -> str:
    if ":" not in line:
        return line
    ...
    *,
    ...
    for path in root.rglob("*"):
        ...
        if any(part in DEFAULT_EXCLUDED_DIRECTORIES for part in relative_parts[:-1]):
            continue
        relative = path.relative_to(root).as_posix()
        if relative in DEFAULT_EXCLUDED_FILES or path.name in DEFAULT_EXCLUDED_FILES:
            continue
