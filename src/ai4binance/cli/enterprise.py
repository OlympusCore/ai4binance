...
from ai4binance.enterprise.pilots import (
    ...
    write_pilot_readiness_artifact,
)
from ai4binance.enterprise.pilots import (
    report_to_payload as pilot_report_to_payload,
)
...
def run_virtual_market_paper_soak_command(*, output_format: str) -> int:
    payload = virtual_market_paper_soak_payload()
    _print_payload(
        payload,
        output_format=output_format,
        command="virtual-market-paper-soak",
    )
    return 0
...
def virtual_market_paper_soak_payload() -> dict[str, object]:
    now = datetime.now(UTC)
    run_suffix = uuid4().hex[:12]
    identity = WorkflowIdentity(
        work_order_id=f"wo:virtual-market-paper-soak:{run_suffix}",
        run_id=f"run:virtual-market-paper-soak:{run_suffix}",
        trace_id=f"trace:virtual-market-paper-soak:{run_suffix}",
        created_at=now,
    )
    report = build_paper_soak_readiness(
        identity,
        pilot_id="virtual-market-paper-soak",
        candidate_ref="virtual_market:paper_soak",
        evidence_refs=(
    ...
    artifact_path = write_pilot_readiness_artifact(
        report,
        Path.cwd()
        / "runtime"
        / "artifacts"
    ...
    payload = pilot_report_to_payload(report)
    return {
        "command": "virtual-market-paper-soak",
        "status": "READY",
        "report": payload,
