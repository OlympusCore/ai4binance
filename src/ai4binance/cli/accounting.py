...
_ACCOUNTING_REST_FAILURE_BUDGET = 3
_ACCOUNTING_REST_MAX_BACKOFF_SECONDS = 300.0
    ...
    *,
    ...
    while max_cycles is None or attempts < max_cycles:
        ...
        try:
            ...
            circuit_open = consecutive_failures >= _ACCOUNTING_REST_FAILURE_BUDGET
            payload = _accounting_daemon_failure_payload(
                "accounting",
                error,
                ...
                next_retry_seconds=(
                    None
                    if circuit_open
                    else _accounting_daemon_backoff_seconds(
                        settings,
            ...
            if max_cycles is not None and attempts >= max_cycles:
                break
            time.sleep(
                _accounting_daemon_backoff_seconds(settings, consecutive_failures)
            )
            continue
        consecutive_failures = 0
        print(json.dumps(to_primitive(payload), ensure_ascii=False, sort_keys=True))
    ...
    return 0 if attempts else 2


def _accounting_daemon_failure_payload(
    service: str,
    error: ExchangeError,
    *,
    ...
    return {
        "command": f"{service}-collect-daemon",
        ...
        "blockers": (
            (
                "ACCOUNTING_PROVIDER_CIRCUIT_OPEN"
                if circuit_open
                else "ACCOUNTING_PROVIDER_CYCLE_FAILED"
            ),
        ),
        "execution_allowed": False,
        "live_eligibility_status": "LIVE_ORDER_BLOCKED",
...
def _accounting_daemon_backoff_seconds(
    settings: Settings,
    consecutive_failures: int,
) -> float:
    retry_multiplier = float(2 ** (consecutive_failures - 1))
    return min(
        settings.runtime_cycle_interval_seconds * retry_multiplier,
        _ACCOUNTING_REST_MAX_BACKOFF_SECONDS,
    )
