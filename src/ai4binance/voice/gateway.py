...
class VoiceIntentParser:
    wake_phrase: str = TR_VOICE_WAKE_PHRASE
    phrases: tuple[tuple[str, VoiceIntent], ...] = tuple(
        (phrase, VoiceIntent(intent_name))
        for phrase, intent_name in TR_VOICE_COMMAND_PHRASES.items()
    )
    ...
    def handle_local(
        ...
        if not session.accepted:
            return VoiceCommandResult(
                False,
                None,
                TR_VOICE_MESSAGES.local_session_failed,
                session.blockers,
            )
        ...
        if verification is None:
            ...
            return VoiceCommandResult(
                False,
                None,
                TR_VOICE_MESSAGES.owner_verification_unavailable,
        ...
        if not verification.accepted:
            return VoiceCommandResult(
                False,
                None,
                TR_VOICE_MESSAGES.owner_verification_failed,
                verification.blockers,
            )
        return self._respond(transcript)
    ...
    def _respond(self, transcript: str) -> VoiceCommandResult:
        if not self.parser.has_wake_phrase(transcript):
            return VoiceCommandResult(
                False,
                None,
                TR_VOICE_MESSAGES.wake_phrase_required,
        ...
        if intent is None:
            return VoiceCommandResult(
                False,
                None,
                TR_VOICE_MESSAGES.intent_not_allowed,
        ...
        if not response:
            return VoiceCommandResult(
                False,
                intent,
                TR_VOICE_MESSAGES.voice_response_unavailable,
