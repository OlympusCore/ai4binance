...
class VoiceResponseBuilder:
    ...
    def respond(self, intent: VoiceIntent) -> str:
        if intent is VoiceIntent.REPEAT:
            return self.last_response or "Henüz okunmuş bir rapor yok."
        if intent is VoiceIntent.STOP_LISTENING:
            return "Sesli asistan dinlemeyi durduruyor."
        if intent is VoiceIntent.MUTE:
            return "Sesli raporlama sessize alındı."
        try:
            payload = self.state_reader.read()
            response = self._build(intent, payload)
        except ValueError:
            response = "Read-only hesap raporu şu anda kullanılamıyor."
    ...
    def _build(self, intent: VoiceIntent, payload: Mapping[str, object]) -> str:
        if intent is VoiceIntent.SYSTEM_STATUS:
            ...
            return (
                f"Sistem {payload.get('state', 'UNKNOWN')}. "
                f"Spot cüzdan {spot.get('wallet_status', 'UNKNOWN')}. "
                f"Futures cüzdan {futures.get('wallet_status', 'UNKNOWN')}. "
                f"Engeller: {_join_or_none(blockers)}. Canlı işlem kapalı."
            )
        ...
        if intent is VoiceIntent.INVENTORY:
            ...
            items = [
                f"{item.get('asset', 'UNKNOWN')} toplam {item.get('total', '0')}"
                for item in inventory[:10]
            ]
            return "Spot envanteri: " + _join_or_none(items)
        if intent is VoiceIntent.FUTURES_POSITIONS:
            positions = _mappings(payload.get("futures_positions"))
            items = [
                f"{item.get('symbol', 'UNKNOWN')} {item.get('side', 'UNKNOWN')} "
                f"miktar {item.get('quantity', '0')}, gerçekleşmemiş kar zarar "
                f"{item.get('unrealized_pnl', '0')}"
                for item in positions[:10]
            ]
            return "Futures pozisyonları: " + _join_or_none(items)
        if intent is VoiceIntent.OPEN_ORDERS:
            orders = _mappings(payload.get("open_orders"))
            items = [
                f"{item.get('market', 'UNKNOWN')} {item.get('symbol', 'UNKNOWN')} "
                f"{item.get('side', 'UNKNOWN')} durum {item.get('status', 'UNKNOWN')}, "
                f"kalan {item.get('remaining_quantity', '0')}"
                for item in orders[:10]
            ]
            return "Açık emirler: " + _join_or_none(items)
        blockers = _texts(payload.get("blockers"))
        return "İşlem engelleri: " + _join_or_none(blockers)
...
def _market_outlook(label: str, market: Mapping[str, object]) -> str:
    ...
    return (
        f"{label} önerisi {market.get('action', 'NO_TRADE')}. "
        f"Yön {market.get('bias', 'UNKNOWN')}. "
        f"Engeller {_join_or_none(blockers)}. Bu yalnız read-only öneridir."
    )
...
class VoiceHealthStore:
    path: Path = Path("State/private/voice-health.json")
