...
def test_support_resistance_metadata_is_structured_and_readable() -> None:
    registry = build_default_registry()
    agent = build_core_technical_agent(registry.get("support_resistance"))
    assert agent is not None

    result = agent.analyze(technical_snapshot(), {})

    assert result.status is AgentStatus.SUCCESS
    assert result.evidence == ("ROLLING_SUPPORT_RESISTANCE",)
    assert result.calculation_metadata is not None
    assert result.calculation_metadata["method"] == "ROLLING_SUPPORT_RESISTANCE"
    assert result.calculation_metadata["timeframe_count"] == 5
    assert result.calculation_metadata["window_bars"] == {
        "5m": 50,
        "15m": 50,
        "1h": 50,
        "4h": 50,
    ...
    calculation_metadata = cast(dict[str, Any], result.calculation_metadata)
    one_hour = calculation_metadata["timeframes"]["1h"]
    assert one_hour["support"] == "1.017"
    assert one_hour["resistance"] == "1.120"
    assert one_hour["current_close"] == "1.118"
    assert one_hour["bias"] == "NEAR_RESISTANCE"
    assert one_hour["window_bars"] == 50
