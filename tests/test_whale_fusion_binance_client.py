...
def test_client_collects_paginated_exact_replay_history_without_filling_gaps() -> None:
    start = datetime(2026, 7, 1, tzinfo=UTC)
    end = start + timedelta(minutes=5 * 501)
    ...
    class ReplayTransport:
        ...
        def get_json(
            ...
            supplied = dict(params or {})
            self.calls.append((path, supplied))
            cursor = int(supplied["startTime"])
            end_milliseconds = int(supplied["endTime"]) + 1
            timestamps = tuple(
                value for value in range(cursor, end_milliseconds, 5 * 60_000)
            )[:500]
            if path == "/futures/data/openInterestHist":
                return [
                    {
            ...
            if path == "/fapi/v1/markPriceKlines":
                return [
                    [timestamp, "1", "1", "1", "101", "0"] for timestamp in timestamps
                ]
            if path == "/fapi/v1/fundingRate":
                return [{"fundingTime": cursor, "fundingRate": "0.0001"}]
            raise AssertionError(path)

    transport = ReplayTransport()
    dataset = BinanceUsdMClient(transport, clock=lambda: NOW).replay_history(
        "BTCUSDT",
        "5m",
        start,
        end,
    ...
    assert len(dataset.series[DerivativesMetric.OPEN_INTEREST]) == 501
    assert len(dataset.series[DerivativesMetric.MARK_PRICE]) == 501
    assert dataset.values(DerivativesMetric.OPEN_INTEREST)[-1] == Decimal("100")
    ...
    assert oi_pages[1]["startTime"] == int(
        (start + timedelta(minutes=5 * 500)).timestamp() * 1000
    )
