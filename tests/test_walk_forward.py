...
def test_rolling_walk_forward_selects_on_train_and_stages_stable_oos() -> None:
    ...
    for fold in report.folds:
        for result in (fold.training_result, fold.oos_result):
            ...
            assert result.false_breakouts == ()
            assert result.trade_outcomes == ()
            assert result.missed_opportunity_ledger.records == ()
...
def test_walk_forward_report_persists_to_redacted_jsonl(tmp_path: Path) -> None:
    ...
    assert stored["detail_level"] == "BOUNDED_SUMMARY"
    assert stored["execution_allowed"] is False
    assert stored["live_eligibility_status"] == "LIVE_ORDER_BLOCKED"
    assert "training_result" not in stored["folds"][0]
    assert "oos_result" not in stored["folds"][0]
    assert stored["folds"][0]["training_metrics"] == to_primitive(
        report.folds[0].training_result.metrics
    )
    assert len(output.read_bytes()) <= JsonlAuditStore(output).max_event_bytes
