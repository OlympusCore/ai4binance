...
def test_regime_candidates_use_requested_candles_and_unique_timeframe_identity() -> (
    None
):
    from ai4binance.strategies.engine import StrategyEngine
    from tests.test_strategy_risk import market_regime_result

    candles = _range_candles()
    market = replace(
        _market(candles),
        timeframes=("15m", "1h"),
        ohlcv_by_timeframe={"15m": candles, "1h": candles},
    )
    results = _agents()
    results["market_regime"] = market_regime_result("RANGE", vote=0.0)
    candidates = StrategyEngine(candidate_timeframes=market.timeframes).generate(
        market, results
    )
    ranges = tuple(item for item in candidates if item.setup_name == "range_rotation")
    assert {item.timeframe for item in ranges} == {"15m", "1h"}
    assert len({item.candidate_id for item in ranges}) == 2
    assert all(
        item.promotion_status is ValidationStatus.RESEARCH_ONLY for item in ranges
    )
    invalid = replace(market, ohlcv_by_timeframe={"15m": candles[:10], "1h": candles})
    candidates = StrategyEngine(candidate_timeframes=market.timeframes).generate(
        invalid, results
    )
    assert all(
        ...
        if item.setup_name == "range_rotation"
    )
