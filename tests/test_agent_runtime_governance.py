...
from ai4binance.ops.jobs import (
    ...
    local_advisory_fixture_job_manifest,
...
def markdown_governance_artifact(
    *,
    ...
    source_path: str = "docs/architecture/framework_architecture_overview.md",
    ...
    declared_authority: str = "RESEARCH_ONLY",
    citations: tuple[str, ...] = (
        "artifact://docs/architecture/framework_architecture_overview.md",
    ),
...
def graph_manifest(
    *,
    ...
    expected_artifacts: tuple[str, ...] = (
        "runtime/artifacts/decisions/opportunity_diamond/reducer-inputs.sha256",
...
def branch_review(
    ...
    *,
    ...
    return OpportunityArtifactReviewResult(
        branch_id=branch_id,
        artifact_uri=f"runtime/artifacts/decisions/opportunity_diamond/{branch_id}.json",
...
def loop_evidence() -> LoopEngineeringReadinessEvidence:
    return LoopEngineeringReadinessEvidence(
        ...
        artifact_log_uri="runtime/artifacts/assurance/loops/opportunity-loop-v1.jsonl",
        holdout_artifact_uri="runtime/artifacts/research/backtest/holdout/HOTUSDT/holdout-report.json",
        oos_artifact_uri="runtime/artifacts/research/backtest/oos/HOTUSDT/walk-forward-report.json",
        human_gate=True,
...
def closed_loop_evidence() -> ClosedLoopAdmissionEvidence:
    return ClosedLoopAdmissionEvidence(
        ...
        verifier_identity="deterministic-quality_gate",
        artifact_log_uri="runtime/artifacts/assurance/loops/research-quality-loop-v1.jsonl",
        stop_conditions=("max_iterations_reached", "quality_gate_passed_or_failed"),
        max_iterations=3,
        max_cost_units=20,
        human_handoff=True,
...
def graph_dependency_integrity_review(
    *,
    ...
    anchor_artifacts: tuple[str, ...] = ("quality_gate:passed",),
...
def closed_loop_run_evidence(
    *,
    ...
    return ClosedLoopRunEvidence(
        ...
        state_uri="runtime/artifacts/assurance/loops/research-quality-loop-v1.state.json",
...
def test_runner_admission_builds_run_card_evidence_and_audit_chain(
    tmp_path: Path,
) -> None:
    runner_manifest = nightly_quality_runner_manifest(tmp_path, tmp_path / "artifacts")
    request = JobRequest(
        job_id="nightly-quality-triage",
        idempotency_key="2026-08-31T0100Z",
    ...
    report = assess_runner_admission(runner_manifest, request, observed_at=NOW)

    assert report.status is RunnerAdmissionStatus.ADMITTED_REPORT_ONLY
    assert report.authority_ceiling is JobAuthorityCeiling.REPORT_ONLY
    assert report.admission.admitted is True
    assert report.blockers == ()
    assert report.run_card.command_ref.endswith(".\\scripts\\quality.ps1")
    assert report.persistent_evidence.artifact_path == (
        tmp_path / "artifacts" / "nightly-quality-triage-2026-08-31T0100Z.json"
    )
    assert report.persistent_evidence.audit_path == (
        tmp_path / "runtime" / "logs" / "nightly-quality-triage-2026-08-31T0100Z.jsonl"
    )
    assert report.audit_record.event_type == "RUNNER_ADMISSION_ACCEPTED"
    assert "LIVE_ORDER_BLOCKED" in report.audit_record.blockers
    assert report.execution_allowed is False
    assert report.live_eligibility_status == "LIVE_ORDER_BLOCKED"


def test_runner_admission_blocks_side_effect_and_command_drift(
    tmp_path: Path,
) -> None:
    capability_manifest = CapabilityManifest(
        manifest_id="runner-manifest:test",
        job_id="nightly-quality",
        allowed_capabilities=(
            JobCapability.READ_REPOSITORY,
            JobCapability.RUN_FIXED_QUALITY_COMMANDS,
        ...
        fixed_command_refs=("allowed-command",),
    )
    runner_manifest = RunnerManifest(
        runner_id="runner:test",
        job_manifest=job_manifest(tmp_path),
    ...
    request = JobRequest(
        ...
        command_ref="disallowed-command",
    )

    report = assess_runner_admission(runner_manifest, request, observed_at=NOW)

    assert report.status is RunnerAdmissionStatus.BLOCKED
    assert report.blockers == (
        "RUNNER_IDENTITY_MISMATCH",
        "RUNNER_SIDE_EFFECT_REQUIRED",
        "RUNNER_COMMAND_NOT_ALLOWLISTED",
    )
    assert report.audit_record.event_type == "RUNNER_ADMISSION_BLOCKED"
    assert report.execution_allowed is False


def test_local_advisory_fixture_runner_manifest_is_singleton_and_research_only(
    tmp_path: Path,
) -> None:
    runner_manifest = local_advisory_fixture_runner_manifest(
        tmp_path,
        tmp_path / "artifacts",
    )
    request = JobRequest(
    ...
    assert runner_manifest.job_manifest.maximum_concurrency == 1
    assert runner_manifest.job_manifest.network_allowed is False
    assert runner_manifest.job_manifest.secret_access_allowed is False
    assert runner_manifest.job_manifest.trading_authority is False
    assert report.status is RunnerAdmissionStatus.ADMITTED_RESEARCH_ONLY
    assert report.authority_ceiling is JobAuthorityCeiling.RESEARCH_ONLY
    assert report.blockers == ()
    assert report.execution_allowed is False
...
def test_local_advisory_fixture_runner_blocks_network_and_command_drift(
    tmp_path: Path,
) -> None:
    job_manifest = local_advisory_fixture_job_manifest(tmp_path, tmp_path / "artifacts")
    report = assess_runner_admission(
        local_advisory_fixture_runner_manifest(tmp_path, tmp_path / "artifacts"),
        JobRequest(
            job_id=job_manifest.job_id,
            idempotency_key="fixture-batch-2",
            requested_capabilities=(JobCapability.RUN_LOCAL_ADVISORY_FIXTURES,),
            target_paths=(tmp_path,),
            timeout_enforced_by_runner=True,
            lock_acquired=True,
            network_requested=True,
            requested_side_effects=(JobSideEffect.RUN_LOCAL_ADVISORY_FIXTURES,),
            runner_id="runner:local-advisory-fixture-evaluation",
            command_ref="other-runner",
        ),
        observed_at=NOW,
    )
...
def test_advisory_fixture_runner_redacts_provider_input_and_stays_advisory_only() -> (
    None
):
    class _Provider:
        prompt = ""
        ...
        def run_fixture(
            ...
            *,
            ...
            assert fixture_id == "fixture-redacted"
            self.prompt = prompt
            return AdvisoryFixtureProviderResponse(
                model_id="loopback-test-model",
                response_text="Citation accepted.",
    ...
    result = run_advisory_fixture(
        AdvisoryFixture(
            fixture_id="fixture-redacted",
            prompt="api_key=raw-secret; evaluate this advisory-only fixture",
            prompt_revision="prompt-v1",
            expectation=AdvisoryEvalExpectation(
                fixture_id="fixture-redacted",
                required_citations=("artifact-1",),
    ...
    assert "raw-secret" not in provider.prompt
    assert "[REDACTED]" in provider.prompt
    assert result.status == "PASS"
    assert result.trace is not None
    assert result.trace.redacted is True
    assert result.execution_allowed is False
    assert result.promotion_evidence is False
    assert result.live_eligibility_status == "LIVE_ORDER_BLOCKED"


def test_advisory_fixture_runner_fails_closed_when_provider_is_unavailable() -> None:
    class _UnavailableProvider:
        def run_fixture(
            ...
            *,
            ...
            del prompt, fixture_id
            raise OSError("local provider unavailable")

    result = run_advisory_fixture(
        AdvisoryFixture(
            fixture_id="fixture-unavailable",
            prompt="evaluate this fixture",
            prompt_revision="prompt-v1",
            expectation=AdvisoryEvalExpectation(
                fixture_id="fixture-unavailable",
                required_citations=(),
    ...
    assert result.blockers == ("ADVISORY_PROVIDER_UNAVAILABLE",)
    assert result.execution_allowed is False


def test_advisory_fixture_runner_rejects_an_invalid_provider_response() -> None:
    ...
    result = run_advisory_fixture(
        AdvisoryFixture(
            fixture_id="fixture-invalid-response",
            prompt="evaluate this fixture",
            prompt_revision="prompt-v1",
            expectation=AdvisoryEvalExpectation(
                fixture_id="fixture-invalid-response",
    ...
    assert result.status == "BLOCKED"
    assert result.blockers == ("ADVISORY_PROVIDER_RESPONSE_INVALID",)
    assert result.execution_allowed is False


def test_advisory_fixture_runner_rejects_a_provider_response_marked_unaccepted() -> (
    None
):
    class _RejectedProvider:
        def run_fixture(
            ...
            *,
            ...
            return AdvisoryFixtureProviderResponse(
                model_id="loopback-test-model",
                response_text="Incomplete response.",
                citations=(),
                blockers=("LOCAL_LLM_RESPONSE_TRUNCATED",),
                finished_at=NOW + timedelta(milliseconds=1),
                accepted=False,
            )
    ...
    result = run_advisory_fixture(
        AdvisoryFixture(
            fixture_id="fixture-rejected-response",
            prompt="evaluate this fixture",
            prompt_revision="prompt-v1",
            expectation=AdvisoryEvalExpectation(
                fixture_id="fixture-rejected-response",
    ...
    assert result.status == "BLOCKED"
    assert result.blockers == ("ADVISORY_PROVIDER_RESPONSE_REJECTED",)
    assert result.execution_allowed is False
...
def test_graph_run_trace_watchlists_checkpoint_identity_shape_drift() -> None:
    graph = opportunity_research_diamond_workflow()
    first_node = graph.nodes[0].node_id
    checkpoints = (
        graph_checkpoint(first_node, run_id="other-run"),
        graph_checkpoint("unknown_branch"),
        graph_checkpoint(first_node, reviewer_result="FAILED"),
        graph_checkpoint(first_node, offset_seconds=2),
    )
    ...
    assert trace.terminal_state is GraphRunTerminalState.WATCHLIST
    assert "GRAPH_RUN_CHECKPOINT_IDENTITY_MISMATCH" in trace.blockers
    assert "GRAPH_RUN_UNKNOWN_NODE_CHECKPOINT" in trace.blockers
    assert "GRAPH_RUN_DUPLICATE_NODE_CHECKPOINT" in trace.blockers
    assert "GRAPH_RUN_MISSING_NODE_CHECKPOINT" in trace.blockers
    assert "GRAPH_RUN_REVIEWER_RESULT_NOT_PASSED" in trace.blockers
...
def test_graph_architecture_readiness_accepts_grounded_graph_as_research_only() -> None:
    ...
    review = review_graph_architecture_readiness(
        GraphArchitectureReadinessEvidence(
            ...
            anchor_artifacts=(
                "runtime/artifacts/research/backtest/validation/HOTUSDT/walk-forward-report.json",
                "runtime/artifacts/decisions/opportunity_diamond/reducer-inputs.sha256",
            ),
            counter_metrics=(
...
def test_graph_workflow_manifest_watchlists_missing_edges_and_unknown_nodes() -> None:
    review = review_graph_workflow_manifest(
        GraphWorkflowManifestEvidence(
            workflow_id="parallel-graph-v0",
            selected_pattern="parallelization",
            nodes=("research", "verify"),
            edges=(),
            cycles_allowed=False,
            parallelism_cap=2,
            expected_artifacts=("runtime/artifacts/graph/verify.json",),
            verifier_nodes=("missing_verifier",),
            human_gate_nodes=("missing_gate",),
    ...
    assert review.status is GraphWorkflowManifestReviewStatus.WATCHLIST
    assert "GRAPH_MANIFEST_EDGE_REQUIRED" in review.blockers
    assert "GRAPH_MANIFEST_VERIFIER_NODE_UNKNOWN" in review.blockers
    assert "GRAPH_MANIFEST_HUMAN_GATE_NODE_UNKNOWN" in review.blockers
    assert "GRAPH_MANIFEST_AUTHORITY_FLAG_REQUIRED" in review.blockers
    assert "LIVE_ORDER_BLOCKED" in review.blockers
...
def test_markdown_governance_review_accepts_canonical_doc_research_only() -> None:
    ...
    assert review.source_path == "docs/architecture/framework_architecture_overview.md"
    assert review.content_sha256 == HASH
    assert review.blockers == ("HUMAN_REVIEW_REQUIRED", "LIVE_ORDER_BLOCKED")
    ...
    reviews = (
        ...
        review_markdown_governance_artifact(
            markdown_governance_artifact(
                ...
                source_path="docs/governance/policy_skills_governance.md",
                role=MarkdownGovernanceArtifactRole.DOMAIN_RULES,
                citations=("artifact://docs/governance/policy_skills_governance.md",),
            )
        ),
    )
...
def test_markdown_governance_reducer_downgrades_missing_or_conflicting_reviews() -> (
    ...
    duplicate = reduce_markdown_governance_reviews(
        (
            ...
            review_markdown_governance_artifact(
                markdown_governance_artifact(
                    source_path="docs/registries/registry_documentation_index.md",
                    citations=(
                        "artifact://docs/registries/registry_documentation_index.md",
                    ),
                )
            ),
        ),
...
def test_graph_dependency_integrity_accepts_anchored_diamond_as_research_only() -> None:
    ...
    assert review.anchor_artifacts == ("quality_gate:passed",)
...
def test_graph_run_checkpoint_rejects_finish_before_start() -> None:
...
def test_graph_node_route_reduction_rejects_decision_observation_mismatch() -> None:
    with pytest.raises(ValueError, match="observations must match decisions"):
        GraphNodeRouteReduction(
            graph_id="graph",
            run_id="run",
            expected_nodes=("plan", "verify"),
            observed_nodes=("plan",),
            missing_nodes=("verify",),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda: CapabilityManifest(
            manifest_id="manifest",
            job_id="job",
            allowed_capabilities=(JobCapability.RUN_FIXED_QUALITY_COMMANDS,),
            authority_ceiling=JobAuthorityCeiling.REPORT_ONLY,
            side_effect_allowlist=(JobSideEffect.RUN_FIXED_QUALITY_COMMANDS,),
            allowed_roots=(Path("C:/repo"),),
        ),
        ...
        lambda: RunnerManifest(
            runner_id="runner",
            job_manifest=job_manifest(Path("C:/repo")),
            capability_manifest=CapabilityManifest(
                manifest_id="manifest",
                job_id="other-job",
                allowed_capabilities=(JobCapability.READ_REPOSITORY,),
                authority_ceiling=JobAuthorityCeiling.REPORT_ONLY,
                side_effect_allowlist=(JobSideEffect.READ_REPOSITORY,),
                allowed_roots=(Path("C:/repo"),),
            ),
            evidence_root=Path("C:/repo/runtime/artifacts"),
            audit_root=Path("C:/repo/runtime/logs"),
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda: MarkdownGovernanceArtifactReview(
            artifact_id="doc",
            source_path="docs/architecture/framework_architecture_overview.md",
            content_sha256=HASH,
        ...
        lambda: ClosedLoopAdmission(
            ...
            artifact_log_uri="runtime/artifacts/assurance/loops/loop.jsonl",
        ...
        lambda: ClosedLoopRunSummary(
            ...
            status=ClosedLoopAdmissionStatus.RESEARCH_ONLY_LOOP,
            state_uri="runtime/artifacts/assurance/loops/loop.state.json",
        ...
        lambda: LoopEngineeringReadinessReview(
            ...
            artifact_log_uri="runtime/artifacts/assurance/loops/loop.jsonl",
            holdout_artifact_uri="runtime/artifacts/research/backtest/holdout/report.json",
            oos_artifact_uri="runtime/artifacts/research/backtest/oos/report.json",
        ),
        lambda: replace(
...
@pytest.mark.parametrize(
    "factory",
    [
        lambda: WorkflowPreview(
            graph_id="graph",
            topological_order=(),
            validation_points=("review",),
        ),
        lambda: WorkflowEdgeAudit(
            graph_id="graph",
            status=WorkflowEdgeAuditStatus.WATCHLIST,
        ...
        lambda: GraphRunTrace(
            ...
            graph_id="graph",
            terminal_state=GraphRunTerminalState.COMPLETED,
            checkpoints=(graph_checkpoint("review"), graph_checkpoint("review")),
            blockers=("HUMAN_REVIEW_REQUIRED", "LIVE_ORDER_BLOCKED"),
            max_cost_units=1,
            max_retry_count=0,
        ),
        lambda: GraphRunTrace(
            ...
            graph_id="graph",
            terminal_state=GraphRunTerminalState.COMPLETED,
            checkpoints=(replace(graph_checkpoint("review"), run_id="other"),),
            blockers=("HUMAN_REVIEW_REQUIRED", "LIVE_ORDER_BLOCKED"),
            max_cost_units=1,
            max_retry_count=0,
        ),
        lambda: GraphRunTrace(
            ...
            graph_id="graph",
            terminal_state=GraphRunTerminalState.COMPLETED,
            checkpoints=(graph_checkpoint("review"),),
            blockers=("HUMAN_REVIEW_REQUIRED",),
            max_cost_units=1,
            max_retry_count=0,
        ),
        ...
        lambda: GraphNodeRouteReduction(
            ...
            run_id="run",
            expected_nodes=("market_research",),
            observed_nodes=("market_research",),
            missing_nodes=(),
            terminal_state=GraphRunTerminalState.COMPLETED,
            decisions=(
                route_decision("market_research"),
                route_decision("market_research"),
            ),
            blockers=("HUMAN_REVIEW_REQUIRED", "LIVE_ORDER_BLOCKED"),
            max_cost_units=1,
        ...
        lambda: GraphNodeRouteReduction(
            ...
            missing_nodes=(),
            terminal_state=GraphRunTerminalState.COMPLETED,
            decisions=(route_decision("market_research"),),
            blockers=("HUMAN_REVIEW_REQUIRED", "LIVE_ORDER_BLOCKED"),
            max_cost_units=1,
            max_retry_count=0,
        ),
        ...
        lambda: AgentArchitectureCoverageSummary(
            workflow_id="workflow",
            source_uri="artifact://summary",
            source_sha256=HASH,
            status=AgentArchitectureLayerStatus.RESEARCH_ONLY_ARCHITECTURE,
            covered_layers=(AgentArchitectureLayer.HARNESS,),
            missing_layers=(AgentArchitectureLayer.HARNESS,),
        ...
        lambda: AgentArchitectureCoverageSummary(
            ...
            source_sha256=HASH,
            status=AgentArchitectureLayerStatus.RESEARCH_ONLY_ARCHITECTURE,
            covered_layers=AGENT_ARCHITECTURE_LAYERS,
            missing_layers=(),
            missing_control_points=("reviewer",),
            blockers=("HUMAN_REVIEW_REQUIRED", "LIVE_ORDER_BLOCKED"),
        ),
        lambda: AgentInvocationCoverageSummary(
            workflow_id="workflow",
            ...
            prompt_revision="rev",
            prompt_sha256=HASH,
            context_sources=("artifact://context",),
            context_sha256=HASH,
            context_freshness_policy="fresh",
            citations=("artifact://citation",),
        ...
        lambda: LoopBuildingBlockCoverageSummary(
            ...
            skills_declared=("quality-gate-loop",),
            connectors_declared=(),
            subagent_roles=("reviewer",),
            memory_sources=("MEMORY.md",),
            cost_budget=1,
            stop_conditions=("green_gate",),
            human_handoff=True,
            status=LoopBuildingBlockCoverageStatus.RESEARCH_ONLY_LOOP,
            blockers=("HUMAN_REVIEW_REQUIRED",),
        ),
        lambda: EvaluatorGateReview(
            ...
            rubric_hash=HASH,
            rubric_text="rubric",
            deterministic_checks=("canary",),
            trajectory_checks=("trace",),
            faithfulness_result="PASSED",
            task_completion_result="PASSED",
            verifier_canary_passed=True,
            blast_radius_lane=EvaluatorGateBlastRadiusLane.REVERSIBLE_CONTAINED,
            human_disagreement_rate=0.0,
            agent_self_assessment_weight=0.0,
            shadow_mode=True,
            blockers=("HUMAN_REVIEW_REQUIRED",),
        ),
        lambda: ResidualEdgeReview(
            ...
            regime_split=("range",),
            signal_decay_check="PASSED",
            data_leakage_check="PASSED",
            sizing_status="PAPER_ONLY",
        ...
        lambda: CoworkTaskSuitabilityReview(
            ...
            touches_files_apps_or_web=True,
            tedious_repetitive_or_multistep=True,
            done_is_checkable=True,
            mistake_is_survivable=True,
            brain_file_present=True,
            skill_declared=True,
            connector_scope=("repo",),
            ...
            trading_scope_touched=False,
            money_movement_touched=False,
            blockers=("HUMAN_REVIEW_REQUIRED",),
        ),
        lambda: AgentTrajectoryLearningEligibilityReview(
            workflow_id="workflow",
            ...
            trajectory_id="trace",
            trajectory_hash=HASH,
            replay_class="DETERMINISTIC_REPLAY",
            privacy_classification="INTERNAL_REDACTED",
            redaction_status="REDACTED",
            reward_signal="DELAYED",
            intervention_candidate="NO_OP",
            tool_schema_version="v1",
            retrieval_snapshot_id="snapshot",
            harness_fingerprint="harness",
            model_id="model",
            causal_step_count=-1,
            delayed_reward_supported=True,
            provenance_versioned=True,
            governance_metadata_present=True,
            human_correction_present=True,
            training_eligible=True,
            secret_risk_detected=False,
            trading_scope_touched=False,
            money_movement_touched=False,
            model_weight_update_requested=False,
            blockers=(
                "MODEL_UPDATE_BLOCKED",
                "HUMAN_REVIEW_REQUIRED",
                "LIVE_ORDER_BLOCKED",
        ...
        lambda: GraphDependencyIntegrityReview(
            ...
            missing_nodes=(),
            declared_edges=("a->b",),
            data_carrying_edges=("b->c",),
            fake_edges=(),
            fanout_width=2,
            barrier_count=1,
            verifier_context_isolated=True,
            anchor_artifacts=("artifact",),
            merge_input_count_verified=True,
            model_tiering_policy_present=True,
            cost_cap_present=True,
            silent_node_failure_count=0,
            trading_scope_touched=False,
            money_movement_touched=False,
            blockers=("HUMAN_REVIEW_REQUIRED", "LIVE_ORDER_BLOCKED"),
        ),
        lambda: AgentModelCandidateReview(
            ...
            tool_call_support=True,
            json_mode_support=True,
            data_privacy_boundary="external",
        ...
        lambda: AgentWorkspaceComponentReview(
            component_id="component",
            component_type=AgentWorkspaceComponentType.SKILL,
            source_path="C:/outside",
            content_sha256=HASH,
            status=AgentWorkspaceComponentStatus.RESEARCH_ONLY_WORKSPACE,
            declared_authority="READ_ONLY",
            canary_command="pytest",
            canary_expected_blockers=("LIVE_ORDER_BLOCKED",),
            citations=("artifact://component",),
            blockers=("HUMAN_REVIEW_REQUIRED", "LIVE_ORDER_BLOCKED"),
        ),
        lambda: MarkdownGovernanceArtifactEvidence(
            artifact_id="artifact",
            source_path="docs/governance/not-md.txt",
            content_sha256=HASH,
            role=MarkdownGovernanceArtifactRole.ARCHITECTURE,
            reviewer_result=MarkdownGovernanceReviewerResult.PASSED,
            declared_authority="REPOSITORY",
            citations=("artifact://doc",),
            owner="Governance",
            freshness_policy="review",
            canonical_source_path="",
            blockers=(),
        ),
        lambda: MarkdownGovernanceArtifactReview(
            artifact_id="artifact",
            source_path="docs/governance/doc.md",
            content_sha256=HASH,
            role=MarkdownGovernanceArtifactRole.ARCHITECTURE,
            status=MarkdownGovernanceReviewStatus.RESEARCH_ONLY_DOCUMENTATION,
            reviewer_result=MarkdownGovernanceReviewerResult.PASSED,
        ...
        lambda: MarkdownGovernanceReviewReduction(
            expected_artifacts=("doc",),
            observed_artifacts=("other",),
            missing_artifacts=(),
            final_status=MarkdownGovernanceReviewStatus.WATCHLIST,
            blockers=("LIVE_ORDER_BLOCKED",),
            reviews=(),
        ),
        lambda: OpportunityArtifactReviewResult(
            branch_id="branch",
            artifact_uri="artifact://branch",
            content_sha256=HASH,
            citations=("artifact://branch",),
            blockers=("BLOCKER",),
            reviewer_result=OpportunityArtifactReviewStatus.PASSED,
        ),
        lambda: ClosedLoopRunEvidence(
            admission=admit_closed_loop(closed_loop_evidence()),
            run_id="run",
            trigger_type=ClosedLoopRunTriggerType.GOAL,
            state_uri="runtime/loop/state.json",
            resume_point="start",
            verifier_identity="verifier",
            started_at=NOW,
            finished_at=NOW - timedelta(seconds=1),
        ...
        lambda: LoopEngineeringReadinessEvidence(
            loop_id="loop",
            task="task",
            selected_pattern="pattern",
            objective_metric="coverage",
            deterministic_verifier=True,
            max_iterations=1,
            max_cost_units=1,
            rollback_or_no_write=True,
            artifact_log_uri="artifacts/loop/log.jsonl",
            holdout_artifact_uri="artifacts/loop/holdout.json",
            oos_artifact_uri="artifacts/loop/oos.json",
            human_gate=True,
            stop_conditions=("green",),
            before_after_metrics=("coverage",),
            allowed_write_roots=("C:/outside",),
        ),
    ],
)
def test_workflow_contract_rejects_additional_guardrail_shapes(
    factory: Callable[[], object],
) -> None:
    with pytest.raises(ValueError, match=r"."):
        factory()
...
def test_nightly_quality_manifest_matches_existing_safe_loop(tmp_path: Path) -> None:
    ...
    assert (
        runner_manifest.capability_manifest.authority_ceiling
        is JobAuthorityCeiling.REPORT_ONLY
    )
    assert "RUN_CARD" in runner_manifest.capability_manifest.required_audit_channels
    assert runner_manifest.execution_allowed is False
