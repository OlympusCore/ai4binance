...
def test_opportunity_comparison_is_research_only_when_evidence_is_blocked() -> None:
    holding = (
        CurrentHoldingReviewEngine()
        .review(
            (
    ...
    opportunity = PortfolioOpportunity(
        ...
        Decimal("100"),
        profile("80", "2.2", "40"),
        blockers=("MARKET_DATA_STALE",),
    )

    comparison = OpportunityComparisonEngine().compare(
        holding=holding,
        opportunity=opportunity,
        free_cash_usdt=Decimal("0"),
    )

    assert comparison.decision is OpportunityComparisonDecision.RESEARCH_ONLY
    assert comparison.reason_codes == ("BLOCKED_EVIDENCE",)
    assert "HOLDING_DATA_STALE" in comparison.blockers
    assert "MARKET_DATA_STALE" in comparison.blockers


def test_opportunity_comparison_keeps_current_holding_when_it_is_stronger() -> None:
    holding = (
        CurrentHoldingReviewEngine()
        .review(
            (
    ...
    assert comparison.decision is OpportunityComparisonDecision.KEEP_CURRENT_HOLDING
    assert comparison.manual_approval_required is False
    assert comparison.reason_codes == ("CURRENT_HOLDING_REMAINS_STRONGER",)


def test_opportunity_comparison_waits_when_advantage_is_insufficient() -> None:
    holding = (
        CurrentHoldingReviewEngine()
        .review(
            (
    ...
    assert comparison.decision is OpportunityComparisonDecision.WAIT_FOR_BETTER_ENTRY
    assert comparison.manual_approval_required is False
    assert comparison.reason_codes == ("OPPORTUNITY_ADVANTAGE_INSUFFICIENT",)


def test_opportunity_comparison_contracts_reject_invalid_inputs() -> None:
    with pytest.raises(ValueError, match="must be finite and non-negative"):
        OpportunityComparisonEngine().compare(
            holding=(
                CurrentHoldingReviewEngine()
                .review(
                    (
            ...
            free_cash_usdt=Decimal("-1"),
        )

    with pytest.raises(ValueError, match="must be non-negative"):
        OpportunityComparisonPolicy(minimum_quality_advantage=Decimal("-1"))

    with pytest.raises(ValueError, match="reduction policy is invalid"):
        OpportunityComparisonPolicy(
            maximum_reduction_pct=Decimal("0.10"),
            minimum_reduction_pct=Decimal("0.20"),
        )
    ...
    with pytest.raises(ValueError, match="identity is required"):
        OpportunityComparison(
            ...
            Decimal("1.0"),
            Decimal("2.0"),
            OpportunityComparisonDecision.NO_ACTION,
        )
    ...
    with pytest.raises(ValueError, match="cannot exceed one"):
        OpportunityComparison(
            ...
            Decimal("2.0"),
            OpportunityComparisonDecision.NO_ACTION,
            suggested_reduction_pct=Decimal("1.1"),
        )
...
def test_scan_cli_commands_are_available_and_blocked(
    ...
    def fake_scan_payload(
        ...
        return {
            ...
            "rejected_symbols": [],
            "blockers": ["SCANNER_INPUT_UNAVAILABLE", "LIVE_ORDER_BLOCKED"],
            "execution_allowed": False,
            "live_eligibility_status": "LIVE_ORDER_BLOCKED",
        }

    monkeypatch.setattr("ai4binance.cli.scan_command_payload", fake_scan_payload)
    ...
    assert "SCANNER_INPUT_UNAVAILABLE" in payload["blockers"]
    ...
    assert alias_payload["rejected_symbols"] == []
