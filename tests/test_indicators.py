...
def test_dynamic_trend_channel_tracks_uptrend_with_atr_width() -> None:
    rows = candles(40, step=Decimal("1"))
    channel = dynamic_trend_channel(
        rows,
        lookback=5,
        atr_period=3,
        width_multiplier=Decimal("2"),
    )

    assert len(channel) == 36
    latest = channel[-1]
    assert latest.timestamp == rows[-1].timestamp
    assert latest.trend_direction == "UP"
    assert latest.slope == Decimal("1")
    assert latest.midline == rows[-1].close
    assert latest.width == Decimal("4")
    assert latest.upper == rows[-1].close + Decimal("4")
    assert latest.lower == rows[-1].close - Decimal("4")
    assert latest.breakout == "INSIDE"


def test_dynamic_trend_channel_classifies_flat_and_thresholded_slopes() -> None:
    flat_rows = candles(10, step=Decimal("0"))
    flat_channel = dynamic_trend_channel(flat_rows, lookback=5, atr_period=3)
    assert flat_channel[-1].trend_direction == "FLAT"
    assert flat_channel[-1].slope == Decimal("0")

    slow_rows = candles(10, step=Decimal("0.01"))
    slow_channel = dynamic_trend_channel(
        slow_rows,
        lookback=5,
        atr_period=3,
        slope_threshold=Decimal("0.05"),
    )
    assert slow_channel[-1].trend_direction == "FLAT"


def test_dynamic_trend_channel_rejects_unsafe_parameters() -> None:
    with pytest.raises(ValueError, match="lookback"):
        dynamic_trend_channel(candles(20), lookback=1)
    with pytest.raises(ValueError, match="atr_period"):
        dynamic_trend_channel(candles(20), atr_period=0)
    with pytest.raises(ValueError, match="width_multiplier"):
        dynamic_trend_channel(candles(20), width_multiplier=Decimal("0"))
    with pytest.raises(ValueError, match="slope_threshold"):
        dynamic_trend_channel(candles(20), slope_threshold=Decimal("-0.1"))
    with pytest.raises(ValueError, match="requires enough closed candles"):
        dynamic_trend_channel(candles(4), lookback=5, atr_period=3)
