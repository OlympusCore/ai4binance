...
def test_normalizer_maps_unknown_chain_and_rejects_scalar_edges() -> None:
    payload: dict[str, object] = {
        "transfer_id": "canonical-2",
        "chain": "unknown-chain",
        "timestamp": 1000,
        "tx_hash": "0xtx",
        "from_address": "0xFrom",
        ...
        "amount": 100,
        "usd_value": None,
        "from_is_new_wallet": False,
        "to_is_new_wallet": False,
        "attributes": {},
    }
    result = CanonicalTransferNormalizer().normalize(payload, provenance=SOURCE)

    assert result.chain is Chain.OTHER
    ...
    for key, value, message in (
        ...
        ("amount", "NaN", "amount"),
        ("from_is_new_wallet", "false", "from_is_new_wallet"),
        ("attributes", (), "attributes"),
    ):
        with pytest.raises(ExchangePayloadError, match=message):
            CanonicalTransferNormalizer().normalize(
                {**payload, key: value},
                provenance=SOURCE,
            )
...
def test_onchain_models_reject_invalid_boundaries() -> None:
    with pytest.raises(ValueError, match="address and label"):
        WalletLabel(
            Chain.ETHEREUM, " ", "label", AddressRole.WHALE, Decimal("0"), SOURCE
        )
    with pytest.raises(ValueError, match="confidence"):
        label("0xwhale", AddressRole.WHALE).__class__(
            Chain.ETHEREUM,
            "0xwhale",
            "label",
            AddressRole.WHALE,
    ...
    with pytest.raises(ValueError, match="usd_value"):
        transfer(usd_value=Decimal("-1"))
    ...
    with pytest.raises(ValueError, match="blockers"):
        OnChainClassification("transfer-1", (classified,), ("BLOCKED",))
    with pytest.raises(ValueError, match="authority"):
        OnChainClassification("transfer-1", (), (), execution_allowed=True)
