...
def required_oek_text() -> str:
    return "\n".join(
        (
            ...
            "OEK does not grant permission for live trading.",
            "Hierarchy of Norms",
...
def test_quality_system_audit_blocks_computer_profile_leak(
    ...
    assert (
        privacy_check.subject_ref
        == "privacy:docs/archive/reference_local_computer_profile.md"
    )
    assert (
        privacy_check.evidence_refs[0]
        == "profile-ref:docs/archive/reference_local_computer_profile.md"
    )
...
def test_oek_constitution_compliance_blocks_missing_gap_analysis_term(
    ...
    weakened_oek = required_oek_text().replace("gap analysis", "")
    root = docs_root(tmp_path, required_doc_text(), oek_text=weakened_oek)

    report = audit_oek_constitution(root)

    assert report.status is OekComplianceStatus.REVISION_REQUIRED
    assert "OEK_GAP_ANALYSIS_TERM_MISSING:gap analysis" in report.blockers
    assert report.execution_allowed is False
...
def test_oek_change_manifest_builder_includes_required_controls_by_default(
    ...
    manifest = build_oek_change_manifest(
        ...
        subject_ref="workflow:quality_gate",
...
def test_oek_gap_analysis_report_rejects_shape_or_authority_drift(
    ...
    document_report = OekComplianceReport(
        document_path="docs/governance/policy_organization_constitution_handbook.md",
...
def test_oek_constitution_compliance_blocks_invalid_frontmatter(
    ...
    invalid_oek = required_oek_text().replace('version: "3.0.0"', 'version: "2.0"')
...
def test_oek_constitution_compliance_blocks_missing_frontmatter(
    ...
    assert report.status is OekComplianceStatus.REVISION_REQUIRED
    assert "OEK_FRONTMATTER_INVALID:canonical_path" in report.blockers
    assert "OEK_FRONTMATTER_INVALID:source_of_truth_scope" in report.blockers
