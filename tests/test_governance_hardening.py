...
def test_development_run_card_writer_persists_atomic_and_audited(
    tmp_path: Path,
) -> None:
    card = DevelopmentRunCard.start(
        run_id="run-1",
        objective="Attach development run cards to persistent evidence storage",
        evidence=evidence(DevelopmentStage.SPEC_REGISTERED),
    )
    writer = DevelopmentRunCardWriter(
        tmp_path / "development-run-card.json",
        JsonlAuditStore(tmp_path / "development-run-cards.jsonl"),
    )

    writer.write(card)

    stored = json.loads(
        (tmp_path / "development-run-card.json").read_text(encoding="utf-8")
    )
    assert stored["run_id"] == "run-1"
    assert stored["execution_allowed"] is False
    assert stored["live_eligibility_status"] == "LIVE_ORDER_BLOCKED"
    assert not (tmp_path / "development-run-card.json.tmp").exists()
    assert "DEVELOPMENT_RUN_CARD_WRITTEN" in (
        tmp_path / "development-run-cards.jsonl"
