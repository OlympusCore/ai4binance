...
def test_artifact_reader_degrades_missing_stale_and_unsafe(tmp_path: Path) -> None:
    ...
    write_artifact(tmp_path, "Artifacts/quality-triage/state.json", unsafe)
...
def test_artifact_reader_degrades_invalid_json_blockers_and_timestamp(
    ...
    path = tmp_path / "Artifacts/quality-triage/state.json"
    path.parent.mkdir(parents=True, exist_ok=True)
    ...
    write_artifact(tmp_path, "Artifacts/quality-triage/state.json", invalid_blockers)
    ...
    write_artifact(tmp_path, "Artifacts/quality-triage/state.json", invalid_timestamp)
    assert reader.read("quality_triage").blockers == (
