...
def test_public_klines_keep_spot_and_futures_routes_separate() -> None:
    transport = FakeTransport()
    client = BinancePublicClient(transport)
    client.klines("HOTUSDT", "5m", 100, market_type="USD_M_FUTURES")
    assert transport.calls[-1][0] == "/fapi/v1/klines"
    client.klines("HOTUSDT", "15m", 100)
    assert transport.calls[-1][0] == "/api/v3/klines"
    with pytest.raises(ValueError, match="market is unsupported"):
        client.klines("HOTUSDT", "1h", 100, market_type="MARGIN")
