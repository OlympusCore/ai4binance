...
from ai4binance.account.account_snapshot import AccountSnapshotBuilder
...
def test_cli_status_helpers_are_fail_closed(
    ...
    assert cli_status.known_stale_temp_paths(tmp_path) == (".pytest-tmp",)
...
def test_advanced_agent_private_feature_branches_are_deterministic() -> None:
    ...
    assert agent._trend_channel(rising, base).evidence == ("LINEAR_CHANNEL_SLOPE",)
    bullish = list(rising)
    bullish[-2] = replace(
        bullish[-2],
        open=Decimal("110"),
    ...
    assert agent._fibonacci(rising, base).metadata is not None
    ...
    assert elliott_feature.evidence == ("RULE_BASED_ALTERNATING_SWINGS",)
...
def test_portfolio_payload_success_and_runtime_failure_are_blocked(
    ...
    assert "FUTURES_ACCOUNT_UNAVAILABLE" in cast(tuple[str, ...], success["blockers"])
    ...
    assert failure["blockers"] == ("PORTFOLIO_RUNTIME_UNAVAILABLE",)
...
def trade_record(net_pnl: str) -> TradeRecord:
    review = ClosureReview(
        exit_reason=BacktestExitReason.END_OF_DATA_EXIT,
