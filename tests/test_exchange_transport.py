...
def test_transport_surfaces_retry_after_without_retrying_rate_limit(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    attempts = 0
    delays: list[float] = []
...
def test_transport_rejects_unsafe_configuration_and_paths() -> None:
    ...
    with pytest.raises(ValueError, match="backoff values"):
        UrllibJsonTransport(backoff_seconds=-0.1)
    with pytest.raises(ValueError, match="too small"):
        UrllibJsonTransport(max_response_bytes=1023)
...
def test_transport_uses_exponential_backoff_for_invalid_retry_after(
    monkeypatch: pytest.MonkeyPatch,
    retry_after: str,
) -> None:
    attempts = 0
    ...
    def fake_urlopen(request: Request, timeout: float) -> FakeResponse:
        ...
        attempts += 1
        if attempts == 1:
            raise HTTPError(request.full_url, 500, "retry", headers, None)
    ...
    assert transport.get_json("/api/v3/time") == {"ok": True}
    assert delays == [0.25]
