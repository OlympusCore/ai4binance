...
from ai4binance.agents.registry import CapabilityRegistry
...
def test_orchestrator_emits_bounded_low_cardinality_metrics() -> None:
    ...
    assert {item.agent_name for item in metrics} == {
        "data_quality",
        "universe_liquidity",
        "confluence",
        "risk",
    ...
    cycle_metrics = telemetry.cycle_snapshot()
    assert len(cycle_metrics) == 1
    cycle_metric = cycle_metrics[0]
    assert cycle_metric.cycle_id == state.snapshot_id
    assert cycle_metric.cycle_wall_ms >= cycle_metric.specialist_wall_ms >= 0.0
    assert cycle_metric.scheduler_overhead_ms >= 0.0
    assert cycle_metric.scheduled_capability_count == len(ORCHESTRATED_SPECIALIST_NAMES)
    skipped = {item.capability_id for item in plan.skipped}
    assert cycle_metric.executed_capability_count == (
        len(ORCHESTRATED_SPECIALIST_NAMES) - len(skipped)
    )
    assert cycle_metric.skipped_capability_count == len(skipped)
    bundle_plan = build_capability_bundle_preflight_plan(
        plan,
        build_default_capability_bundle_registry(),
    )
    assert cycle_metric.scheduled_bundle_count == len(bundle_plan.scheduled_bundle_ids)
    assert cycle_metric.skipped_bundle_count == len(bundle_plan.skipped_bundle_ids)
    assert 0 < cycle_metric.bundle_task_count <= cycle_metric.executed_capability_count


def test_preflight_skips_unavailable_or_expensive_capabilities_without_execution() -> (
    None
):
    registry = build_default_capability_registry()
    standard = build_capability_preflight_plan(
        registry,
        snapshot(),
        mode=CapabilityPreflightMode.STANDARD,
    )
    decisions = {item.capability_id: item for item in standard.decisions}
    assert decisions["order_flow"].scheduled is False
    assert "TIMEFRAME_NOT_APPLICABLE" in decisions["order_flow"].reason_codes
    assert "REQUIRED_DATA_MISSING:order_book" in decisions["order_flow"].reason_codes
    assert decisions["harmonic_pattern"].scheduled is False
    assert decisions["harmonic_pattern"].reason_codes == (
        "DEPENDENCY_NOT_SCHEDULED:fibonacci",
    )

    economy = build_capability_preflight_plan(
        registry,
        snapshot(),
        mode=CapabilityPreflightMode.ECONOMY,
    )
    economy_decisions = {item.capability_id: item for item in economy.decisions}
    assert economy_decisions["chart_pattern"].scheduled is False
    assert economy_decisions["chart_pattern"].reason_codes == (
        "EXPENSIVE_CAPABILITY_DEFERRED",
    )

    state = EnterpriseOrchestrator(
        preflight_mode=CapabilityPreflightMode.ECONOMY,
    ).analyze(snapshot())
    assert state.agent_results["chart_pattern"].status is AgentStatus.NOT_APPLICABLE
    assert state.agent_results["chart_pattern"].reason_codes == (
        "EXPENSIVE_CAPABILITY_DEFERRED",
    )
...
def test_preflight_fails_closed_for_runtime_and_regime_restrictions() -> None:
    registry = build_default_capability_registry()
    restricted_registry = CapabilityRegistry(
        tuple(
            ...
            if definition.capability_id == "trend"
            else definition
            for definition in registry.definitions
        ),
        external_dependency_ids=registry.external_dependency_ids,
    )
    plan = build_capability_preflight_plan(
        restricted_registry,
        snapshot(),
    ...
    trend = next(item for item in plan.decisions if item.capability_id == "trend")
    assert trend.scheduled is False
    assert trend.reason_codes == ("RUNTIME_NOT_ELIGIBLE", "REGIME_UNAVAILABLE")
    with pytest.raises(ValueError, match="cannot carry skip"):
        CapabilityPreflightDecision("trend", scheduled=True, reason_codes=("TEST",))


def test_preflight_groups_capabilities_into_six_visible_runtime_bundles() -> None:
    capability_plan = build_capability_preflight_plan(
        build_default_capability_registry(),
        snapshot(),
        mode=CapabilityPreflightMode.STANDARD,
    )
    ...
    assert len(bundle_plan.decisions) == 6
    assert "cross_market_derivatives" in bundle_plan.skipped_bundle_ids
    assert "market_state" in bundle_plan.scheduled_bundle_ids
    assert {
        capability_id
        for decision in bundle_plan.decisions
        for capability_id in (
            *decision.scheduled_capability_ids,
            *decision.skipped_capability_ids,
        )
    } == set(ORCHESTRATED_SPECIALIST_NAMES)


def test_orchestrator_records_early_exit_without_specialist_capabilities() -> None:
    telemetry = InMemoryAgentTelemetry(capacity=8)
    EnterpriseOrchestrator(
        telemetry_sink=telemetry,
        minimum_candles=10_000,
    ).analyze(snapshot())

    cycle_metric = telemetry.cycle_snapshot()[0]
    assert cycle_metric.scheduled_capability_count == 0
    assert cycle_metric.executed_capability_count == 0
    assert cycle_metric.skipped_capability_count == 0
    assert cycle_metric.specialist_wall_ms == 0.0
...
def test_telemetry_capacity_and_contract_validation() -> None:
    ...
    with pytest.raises(ValueError, match="skipped"):
        OrchestrationCycleMetric(
            ...
            snapshot_id="snapshot",
            cycle_wall_ms=1.0,
            specialist_wall_ms=1.0,
            scheduler_overhead_ms=0.1,
            scheduled_capability_count=2,
            executed_capability_count=1,
            skipped_capability_count=0,
        )
